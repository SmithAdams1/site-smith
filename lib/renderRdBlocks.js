  ...
  rd_prose(d, locale) {
    ...
    const cards = (Array.isArray(d.cards) && d.cards.length)
      ? `\n          <div class="rd-two" style="margin-top:clamp(32px,4vw,56px);">` +
        ...
        `\n          </div>`
      : '';
    // Optional subtle brochure downloads (HoS: "downloadable informative brochures").
    ...
    const dls = (Array.isArray(d.downloads) && d.downloads.length)
      ? '\n            <div class="reveal" style="display:flex;flex-wrap:wrap;gap:26px;margin-top:28px;">' +
        d.downloads.map(x => `<a href="${attr(x.href)}" download style="display:inline-flex;align-items:center;gap:8px;font-size:13px;font-weight:600;letter-spacing:.02em;color:${navy ? '#fff' : 'var(--navy,#11222D)'};text-decoration:none;border-bottom:1px solid var(--champ,#D6C29A);padding-bottom:3px;">${DL_ICON}<span>${pick(x.label, locale)}</span></a>`).join('') +
        '</div>'
      : '';
    return (
