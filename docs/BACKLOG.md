...
| 2026-08-05 (~afternoon) | FAQ + FAQPage schema on /invest-in-portugal (SEO/AEO) | abilio.diz / Opus 4.8 | Branch `main` → prod: `4c9b66e`. 8 positively-framed Q&As (Golden Visa/D2/residency/citizenship/advisor) as an accessible `<details>` accordion (content always in DOM) + matching `FAQPage` JSON-LD. CMS-editable (`invest.faq.q0..7`/`a0..7`). Facts to be validated by legal (visa rules). Google retired FAQ visual rich results for non gov/health in 2023 → value is AEO (ChatGPT/Gemini/AI Overviews) + on-page SEO + conversion. Could add a shorter FAQ to /property-management next. |
| 2026-08-05 (~afternoon) | SEO decision 8 RESOLVED: canonical host = **www** | abilio.diz / Opus 4.8 | Branch `main` → prod: `375fb86`. Site 307-redirects apex→www, so www is the canonical host. Switched all canonicals, sitemap `<loc>`, OG/Twitter, JSON-LD, robots.txt from apex to www (198 refs / 24 files). Reason: GSC www URL-prefix property was rejecting the sitemap because its URLs were apex ("outside property"). Now aligned. |
| 2026-08-05 (~afternoon) | SEO batch: structured data + social + sitemap + H1s + breadcrumbs | abilio.diz / Opus 4.8 | Branch `main` → prod: `a15fe94`. Replaced SVG og:image with branded 1200×630 JPG (`/og-image.jpg`) + Twitter Cards; enriched homepage RealEstateAgent/LocalBusiness JSON-LD (fixed "developers" wording, added address/geo/sameAs/areaServed); `BlogPosting` schema on all 9 blog posts; `BreadcrumbList` on 7 subpages + 9 posts; rewrote sitemap to canonical extensionless URLs + added the 9 blog posts (were missing); `<h1>` on about.html (was 0). 32 JSON-LD blocks validated, verified on prod. **Still open (needs decision):** canonical host apex-vs-www (apex canonical currently 307s to www); PT/hreflang (EN+PT share one URL → PT not indexed). |
| 2026-08-05 (~afternoon) | Homepage P0/P1 audit fixes (media + SEO/UX) | abilio.diz / Opus 4.8 | Branch `main` → prod: `4519e44` + `2595ce4`. Re-encoded both hero videos (53+39MB → 2.4+1.7MB, audio stripped) and resized heaviest images in place (5.png 16→1.9MB, benefitsImage2 7→0.7MB, AML 7.5→0.2MB); added a real `<h1>` (was 0); stats label contrast .4→.72; CTAs on the two advisory cards (CMS keys `index.pillars.{0,1}.cta_{url,label}`, default → invest-in-portugal.html); chat tooltip delay 1.5→4s + em dashes → hyphens; dropped 3 orphan image preloads. **Homepage media ~90MB → ~7MB.** Verified on prod. |
