...
- **Media → NEXT and last.** Upload already works everywhere; finish the shared **browse / pick / reuse / delete** library over `site-media` so every image field (RE gallery, Blog cover, Site-content images) can pick from one place instead of only uploading new files.
- **Legacy files now unreferenced by Studio:** `admin.html`, `admin-real-estate.html` (the iframes are gone). `admin-v2/v3` already deleted. **Retire (delete) these two in one cleanup commit AFTER Media is done and all writes are re-confirmed** — keep them until then as a safety net.
