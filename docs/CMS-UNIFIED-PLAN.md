...
- **Security (Phase 0) — CLOSED (2026-08-08):** owner rotated the `admin@smithandadams.com` password, so the cleartext still in git history (`d6f20c0`) is now a dead credential. History deliberately not rewritten (272-commit rewrite + force-push too risky with concurrent sessions; payoff nil once the password is dead). Repo still public; making it private is optional now.
